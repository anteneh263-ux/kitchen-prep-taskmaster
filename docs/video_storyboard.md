...
| 1:54–2:32 | “Here is the complete autonomous workflow. Operational inputs enter the private worker; Gemini proposes demand; Python validates, applies FEFO, calculates shortages and replenishment, and publishes one replay-safe plan.” | Clean **Autonomous run** sequence followed by the verified execution-result card. No terminal, username or credential surface. | Progress through the five bounded stages, then hold on the published evidence. |
...
| 3:28–3:40 | “The repository includes reproducible deployment instructions and 128 passing tests covering validation, fallback, FEFO, replay safety and public-route security.” | Prepared test-evidence card or GitHub test evidence. | Static proof shot. |
